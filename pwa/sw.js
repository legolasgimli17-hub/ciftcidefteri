...
async function cacheFirst(request,name,maxEntries){const cache=await caches.open(name),hit=await cache.match(request);if(hit)return hit;const response=await fetch(request);if(response?.ok){await cache.put(request,response.clone());if(maxEntries)void trim(name,maxEntries);}return response;}
...
self.addEventListener('fetch',event=>{if(event.request.method!=='GET')return;const url=new URL(event.request.url);if(isLiveData(url)){event.respondWith(networkFirst(event.request,DATA));return;}if(isSatellite(url)){event.respondWith(cacheFirst(event.request,SATELLITE,160));return;}if(isWeather(url)){event.respondWith(networkFirst(event.request,WEATHER));return;}if(isLocalAsset(url)){event.respondWith(cacheFirst(event.request,STATIC));return;}if(url.origin===self.location.origin)event.respondWith(networkFirst(event.request,STATIC).catch(()=>caches.match(event.request).then(hit=>hit||caches.match('/'))));});
