...
{crop:'Mercimek',low:22.82,high:44.83,unit:'kg',date:'2026-08-28',source:'TZOB',kind:'çeşide göre yeşil-kırmızı üretici ortalamaları'},
{crop:'Nohut',ref:32.22,unit:'kg',date:'2026-08-28',source:'TZOB',kind:'üretici ortalaması'},
{crop:'Kuru fasulye',ref:39.86,unit:'kg',date:'2026-08-28',source:'TZOB',kind:'üretici ortalaması'},
{crop:'Domates',ref:17.92,unit:'kg',date:'2026-08-28',source:'TZOB',kind:'üretici ortalaması'},
{crop:'Sivri biber',ref:41.68,unit:'kg',date:'2026-08-28',source:'TZOB',kind:'üretici ortalaması'},
...
{crop:'İç fındık',ref:410.00,unit:'kg',date:'2026-08-28',source:'TZOB',kind:'serbest piyasa referansı'},
...
{crop:'Şeker pancarı',ref:3.10,unit:'kg',date:'2025-09-30',source:'TÜRKŞEKER',kind:'2025-2026 eski sezon baz alım referansı',stale:true}
];
function marketFor(crop){return MARKET.find(x=>x.crop===crop)||null;}
function effectiveReference(entry){if(!entry||entry.stale)return null;if(Number.isFinite(entry.ref))return {low:entry.ref,high:entry.ref,mid:entry.ref};if(Number.isFinite(entry.low)&&Number.isFinite(entry.high))return {low:entry.low,high:entry.high,mid:(entry.low+entry.high)/2};return null;}
function roundTL(value){return Math.round((Number(value)+Number.EPSILON)*100)/100;}
function productValue(quantityKg,entry,overridePrice){const qty=Number(quantityKg);if(!Number.isFinite(qty)||qty<0)throw new Error('invalid_quantity');const custom=Number(overridePrice);if(Number.isFinite(custom)&&custom>0){const total=roundTL(qty*custom);return {low:total,high:total,mid:total,custom:true};}const r=effectiveReference(entry);if(!r)return null;return {low:roundTL(qty*r.low),high:roundTL(qty*r.high),mid:roundTL(qty*r.mid),custom:false};}
function groupExpenseSections(transactions,effective){const map=new Map();for(const t of transactions||[]){if(t.type!=='expense')continue;const key=t.category||'Diğer masraf';if(!map.has(key))map.set(key,{category:key,total:0,items:[]});const group=map.get(key);group.items.push(t);group.total+=Number(effective(t))||0;}return [...map.values()].sort((a,b)=>b.total-a.total);}
function parcelSummaries(transactions,effective){const map=new Map();for(const t of transactions||[]){const parcel=(t.parcel||'').trim();if(!parcel)continue;if(!map.has(parcel))map.set(parcel,{parcel,crop:t.crop||'Genel',income:0,expense:0,count:0,lastDate:'',lastCategory:''});const p=map.get(parcel);p.crop=t.crop||p.crop;p.count++;const value=Number(effective(t))||0;if(t.type==='income')p.income+=value;else p.expense+=value;if(!p.lastDate||String(t.date)>p.lastDate){p.lastDate=t.date||'';p.lastCategory=t.category||'';}}return [...map.values()].map(x=>({...x,result:x.income-x.expense})).sort((a,b)=>String(b.lastDate).localeCompare(String(a.lastDate)));}
