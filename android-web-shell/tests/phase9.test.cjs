...
const localBase=fs.readFileSync(path.join(root,'pwa/assets/index.html'),'utf8');
...
assert.match(pwa,/<title>EkinCep — Tarla defteri ve saha takibi<\/title>/);assert.match(pwa,/meta name="description"/);assert.match(pwa,/property="og:image"/);assert.match(pwa,/class="skeleton"/);assert.match(localBase,/indexedDB/);assert.match(loader,/ekincep-last-weather-v1/);
assert.match(sw,/caches\.open/);assert.match(sw,/api\.open-meteo\.com/);assert.match(sw,/\/assets\/phase9-field-ui\.js/);assert.equal(/raw\.githubusercontent\.com|cdn\.jsdelivr\.net/.test(loader+sw),false);assert.match(manifest,/"short_name":"EkinCep"/);
